// Originally from https://github.com/abbott567/validation-for-prototypes
// This should no longer be in /vendor since it has been hacked about
...
function checkTextFields(errors) {
  $(document).find('input[type="text"], textarea').each(function () {
    ...
    var label = $(this).parent().find('label').clone().children('.js-hidden, input').remove().end().text();

    if ($fieldset.attr('data-required') !== undefined && $(this).val() === '' && !$(this).parent().hasClass('js-hidden') && !$fieldset.parent().hasClass('js-hidden')) {
